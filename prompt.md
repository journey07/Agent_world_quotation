...
- `상세 장소 필드 일방향 동기화 수정` - 문제: "상세 장소"→"상세 설치 장소" 전달 안 됨 → 해결: (1) stateRef.current에 formData 추가, (2) handleSubmit에서 stateRef.current.formData 사용하여 최신 상태 보장, (3) preview-image API도 updatedFormData 사용
...
## 2026-02-02
- `'상세견적서'로 '견적 요약'의 내용이 정확히 전달되지 않아. 특히, 레이아웃을 만들고 각 열의 단 수 or 높이를 바꾸면 견적 요약의 내용이 바로 바뀌거든? 그 내용을 가져와서 견적서를 만들어야해. 또한, 내용이 만아져도 상세견적서는 1페이지여야 하니까 항목들이 늘어나면 2D이미지의 크기를 살짝 조절해서 맞춰야하나? 또한 제어부, 함체, 설치 관련 내용 말고는 행 높이를 살짝 줄여.`
  - 수정 파일: backend/src/services/excelService.js
  - 변경 내용:
    1. lockerBodiesBreakdown 배열 활용 - 열별 단수가 다를 때 함체부를 단수별로 분리 표시
    2. 행 높이 동적 조절 - 제어부/함체/설치는 기본 42pt, 옵션/프로그램은 28pt로 축소
    3. 2D 이미지 크기 동적 조절 - 항목 수 증가 시 이미지 높이 자동 축소 (1페이지 유지)
- `'견적서 다운로드' 모달 디자인 개선` - 3D 경고 모달 리디자인 (글래스모피즘, 부드러운 애니메이션, 정제된 버튼)
- `문의내역에서 '상태' 디자인 고급스럽게 변경` - 상태 배지 리디자인: 아이콘 추가, 글래스 효과, 세련된 드롭다운
