 ...
 * @param {string} userName - Optional user name (from users table name column)
 * @param {string} imageUrl - Optional image URL (for 3D images, etc.)
 */
export async function sendActivityLog(action, logType = 'info', responseTime = 0, userName = null, imageUrl = null) {
    ...
    console.log(`📋 [LOGIN LOG] Parameters: action="${action}", logType="${logType}", userName="${userName || 'null'}", imageUrl="${imageUrl || 'null'}"`);
    ...
    try {
        const payload = {
            ...
            imageUrl: imageUrl || null  // Add image URL to payload
        };
