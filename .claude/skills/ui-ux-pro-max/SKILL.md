---
name: ui-ux-pro-max
description: UI/UX 디자인 시 항상 참조. 컴포넌트 디자인, 색상, 타이포그래피, 레이아웃, 접근성 가이드라인 제공.
---
...
These frequently overlooked issues make UI appear unprofessional.
...
| Rule | Do | Don't |
|------|-----|-------|
| **Cursor pointer** | Add `cursor-pointer` to clickable/hoverable cards | Leave default cursor on interactive elements |
...
1. **Always measure first** - Check current spacing/sizing before changing
2. **Use consistent increments** - 4px or 8px steps
3. **Test at multiple breakpoints** - 375px, 768px, 1024px, 1440px
4. **Verify alignment** - Use browser dev tools grid overlay
5. **Check contrast** - Use browser accessibility tools
